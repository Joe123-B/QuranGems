...
const verseSchema = {
  ...
  properties: {
    ...
    transliteration: {
      type: Type.STRING,
      description: "A phonetical transliteration of the Arabic verse(s) into English letters."
    },
    translation: {
      type: Type.STRING,
  ...
  required: ['reference', 'arabic', 'transliteration', 'translation'],
};
    ...
    switch (action) {
      case 'getPrayerVerses':
        ...
        geminiResponse = await ai.models.generateContent({
          ...
          contents: `Find 1-3 longer Quranic passages or verse compilations that are impactful for prayer related to the following topic: "${query}". These should be suitable for extended recitation during prayer. For each passage, provide the combined Arabic text, transliteration, translation, and a reference that covers the entire range of verses (e.g., 'Surah Al-Imran, 3:190-194').`,
          config: {
            systemInstruction: "You are an expert Islamic scholar. Provide accurate Quranic verses, including the Arabic text, a clear transliteration, an eloquent English translation, and the precise reference (Surah name, chapter:verse number range).",
      ...
      case 'getRandomVerse':
        geminiResponse = await ai.models.generateContent({
          model: "gemini-2.5-flash",
          contents: "Provide one random, beautiful, and impactful passage from the Quran, consisting of about 5-8 verses, that offers wisdom, comfort, or inspiration. For the passage, provide the combined Arabic text, transliteration, translation, and a reference that covers the entire range of verses (e.g., 'Surah Al-Baqarah, 2:285-286').",
          config: {
            systemInstruction: "You are an expert Islamic scholar. Provide one accurate Quranic verse or passage, including the Arabic text, a clear transliteration, an eloquent English translation, and the precise reference (Surah name, chapter:verse number range).",
            responseMimeType: "application/json",
